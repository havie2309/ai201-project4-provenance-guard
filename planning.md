...
**Ensemble Detection (3rd signal):** Adding a lexical pattern-matching signal that counts known AI-cliché phrases (e.g. "it is important to note," "delve into," "paradigm shift") per 100 words, normalized to 0–1. This is genuinely distinct from Signal 1 (holistic LLM judgment) and Signal 2 (statistical structure) because it's a targeted keyword match. Combined via weighted average: LLM 0.5, stylometric 0.3, phrase-pattern 0.2 — phrase-pattern weighted lowest since it's the easiest signal to evade (a single paraphrase pass defeats it) and most prone to false positives on formal human writing that uses these phrases naturally. Same disagreement-dampening approach as the 2-signal case, applied across all three (spread > 0.5 pulls the score toward 0.5).

**Provenance Certificate ("Verified Human"):** A creator earns a "Verified Human" certificate by accumulating at least 2 prior submissions that scored confidently human (confidence <= 0.20) with no open appeal against them. `POST /certify` checks eligibility and issues the certificate; `GET /certificate/<creator_id>` checks status. Once certified, future `/submit` responses for that creator append a note to the transparency label noting their certified status. This doesn't override the detection pipeline — a certified creator can still receive an "uncertain" or "likely_ai" result on a given piece — it's a reputational signal shown alongside the per-submission result, not a bypass.